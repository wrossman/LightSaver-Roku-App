...
sub startGetResource()
    ...
    m.getResourcePackageTask.observeField("result", "finishUploadFlow")
    m.getResourcePackageTask.control = "run"

end sub

sub finishUploadFLow()
    ...
    m.settings.Write("imgSource", "upload")
    m.settings.Write("imgLinks", linksStr)
    m.settings.Flush()

    m.global.imgSource = "upload"

    menu = m.top.getParent()
    menu.removeChild(m.top)
    m.global.currScreen = "UploadSuccess"
end sub
